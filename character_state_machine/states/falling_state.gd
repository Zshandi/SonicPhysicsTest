...
func _physics_process(delta: float) -> void:
	...
	DebugValues.debug("top_speed", top_speed, FALLING_DEBUG)
	DebugValues.debug("abs(ch.velocity.x) < top_speed", abs(ch.velocity.x) < top_speed, FALLING_DEBUG)
	...
	# Air movement
	if abs(ch.velocity.x) < top_speed:
		var velocity_x_delta := air_acceleration * ch.get_input_left_right() * delta
		DebugValues.debug("velocity.x delta", velocity_x_delta, FALLING_DEBUG)
		ch.velocity.x += velocity_x_delta
		ch.velocity.x = clamp(ch.velocity.x, -top_speed, top_speed)
